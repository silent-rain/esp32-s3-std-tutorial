...
impl<'d> Mpu6050<'d> {
    ...
    /// custom sensitivity
    pub fn new_with_sens(i2c: I2cDriver<'d>, arange: AccelRange, grange: GyroRange) -> Self {
        Mpu6050 {
            i2c,
            slave_addr: DEFAULT_SLAVE_ADDR,
    ...
    pub fn new_with_addr(i2c: I2cDriver<'d>, slave_addr: u8) -> Self {
        Mpu6050 {
            i2c,
            slave_addr,
            acc_sensitivity: ACCEL_SENS.0,
    ...
    pub fn set_clock_source(&mut self, source: CLKSEL) -> Result<(), EspError> {
        Ok(self.write_bits(
            PWR_MGMT_1::ADDR,
    ...
    pub fn get_clock_source(&mut self) -> Result<CLKSEL, EspError> {
    ...
    pub fn setup_motion_detection(&mut self) -> Result<(), EspError> {
    ...
    pub fn get_motion_detected(&mut self) -> Result<bool, EspError> {
        Ok(self.read_bit(INT_STATUS::ADDR, INT_STATUS::MOT_INT)? != 0)
    }

    /// set accel high pass filter mode
    pub fn set_accel_hpf(&mut self, mode: ACCEL_HPF) -> Result<(), EspError> {
        Ok(self.write_bits(
            ACCEL_CONFIG::ADDR,
    ...
    pub fn get_accel_hpf(&mut self) -> Result<ACCEL_HPF, EspError> {
    ...
    pub fn set_gyro_range(&mut self, range: GyroRange) -> Result<(), EspError> {
        self.write_bits(
            GYRO_CONFIG::ADDR,
    ...
    pub fn get_gyro_range(&mut self) -> Result<GyroRange, EspError> {
    ...
    pub fn set_accel_range(&mut self, range: AccelRange) -> Result<(), EspError> {
        self.write_bits(
            ACCEL_CONFIG::ADDR,
    ...
    pub fn get_accel_range(&mut self) -> Result<AccelRange, EspError> {
        let byte = self.read_bits(
            ACCEL_CONFIG::ADDR,
            ACCEL_CONFIG::FS_SEL.bit,
            ACCEL_CONFIG::FS_SEL.length,
    ...
    /// reset device
    pub fn reset_device(&mut self) -> Result<(), EspError> {
    ...
    pub fn set_sleep_enabled(&mut self, enable: bool) -> Result<(), EspError> {
        Ok(self.write_bit(PWR_MGMT_1::ADDR, PWR_MGMT_1::SLEEP, enable)?)
    }

    /// get sleep status
    pub fn get_sleep_enabled(&mut self) -> Result<bool, EspError> {
    ...
    pub fn set_temp_enabled(&mut self, enable: bool) -> Result<(), EspError> {
    ...
    pub fn get_acc_angles(&mut self) -> Result<Vector2<f32>, EspError> {
    ...
    fn read_rot(&mut self, reg: u8) -> Result<Vector3<f32>, EspError> {
    ...
    pub fn get_acc(&mut self) -> Result<Vector3<f32>, EspError> {
    ...
    pub fn get_gyro(&mut self) -> Result<Vector3<f32>, EspError> {
    ...
    pub fn get_temp(&mut self) -> Result<f32, EspError> {
    ...
    pub fn write_byte(&mut self, reg: u8, byte: u8) -> Result<(), EspError> {
        self.i2c.write(self.slave_addr, &[reg, byte], BLOCK)?;
    ...
    pub fn write_bit(&mut self, reg: u8, bit_n: u8, enable: bool) -> Result<(), EspError> {
    ...
    fn read_bit(&mut self, reg: u8, bit_n: u8) -> Result<u8, EspError> {
        let mut byte: [u8; 1] = [0; 1];
        self.read_bytes(reg, &mut byte)?;
    ...
    pub fn read_bits(&mut self, reg: u8, start_bit: u8, length: u8) -> Result<u8, EspError> {
        let mut byte: [u8; 1] = [0; 1];
        self.read_bytes(reg, &mut byte)?;
    ...
    pub fn read_byte(&mut self, reg: u8) -> Result<u8, EspError> {
        let mut byte: [u8; 1] = [0; 1];
        self.i2c
            .write_read(self.slave_addr, &[reg], &mut byte, BLOCK)?;
        Ok(byte[0])
    }

    /// Reads series of bytes into buf from specified reg
    pub fn read_bytes(&mut self, reg: u8, buf: &mut [u8]) -> Result<(), EspError> {
        self.i2c.write_read(self.slave_addr, &[reg], buf, BLOCK)?;
        Ok(())
    }
}
